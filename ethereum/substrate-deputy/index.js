...
const run = async () => {
	...
	let get_abi_success = true;
	let abi = await get_contract_abi()
	if (!get_abi_success) {
		logger.error('error, get_contract_abi failed, retry -----');
		process.exit(-1);
	}
    ...
    while (true) {
        ...
        if (latest_block_num <= 0) {
			logger.error('error, get_latest_block_num failed, retry -----');
            continue
        }
		...
		let get_logs_success = true;
		let enent_logs = await get_contract_logs(from, to);
		if (!get_logs_success) {
			logger.error('error, get_contract_logs failed, retry -----');
			continue;
		}

        if (enent_logs != undefined && enent_logs.length > 0) {
            for (index in enent_logs) {
                let topics = enent_logs[index].topics;
                let event_name = event_name_map.get(topics[0]);
                logger.info("got event_name: ", event_name);
            }

            try {
                const nonce = await api.query.system.accountNonce(pair.address);
                let post_data = '{"jsonrpc":"2.0","method":"eth_getLogs","params":[{"address": "' + CONTRACT_ADDRESS + '", "fromBlock":"0x' + from.toString(16) + '","toBlock":"0x' + to.toString(16) + '"}],"id": 1}';
    ...
    async function get_latest_block_num() {
		...
		let url = util.format(API_URL + '/api?module=proxy&action=eth_blockNumber&apikey=%s', API_KEY);
        await new Promise((resolve, reject) => {
            request({method: 'GET', url: url, timeout: 10000,}, function (error, response, data) {
            ...
            logger.error('get_latest_block_num return error')
        })
	...
	*/
    ...
    async function get_contract_logs(fromBlock, toBlock) {
		let logs;
		let url = util.format(API_URL + '/api?module=logs&action=getLogs&fromBlock=%s&toBlock=%s&address=%s&apikey=%s', fromBlock, toBlock, CONTRACT_ADDRESS, API_KEY);
		await new Promise((resolve, reject) => {
            request({method: 'GET', url: url, timeout: 10000,}, function (error, response, data) {
                if (!error && response != null && response.statusCode != null && response.statusCode == 200) {
					let data_json = JSON.parse(data);
			...
			get_logs_success = false;
			logger.info(err, 'get_contract_logs return error, get_logs_success:', get_logs_success);
        })
        return logs;
    }
    ...
    async function get_contract_abi() {
        ...
        await new Promise((resolve, reject) => {
			...
			get_abi_success = false;
            logger.info(err, 'get_contract_abi return error, get_abi_success:', get_abi_success)
        })
        return abi;
    }
}
